  ...
  class FormBuilder < ActionView::Helpers::FormBuilder
    ...
    def ckeditor(name)
      @template.content_for :head do
        @template.javascript_include_tag('ckeditor/ckeditor').html_safe + 
        @template.javascript_include_tag('ckeditor/adapters/jquery').html_safe + 
        @template.raw('<script type="text/javascript" charset="utf-8">
          <!--
            $(function() {
              $(\'#' + @object.class.to_s.downcase + '_' + name.to_s + '\').ckeditor();
            })
          //-->
        </script>').html_safe
      end
      
      input name, :class => 'ckeditor'
    end
